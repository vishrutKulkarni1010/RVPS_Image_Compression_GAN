...
class Network(object):
    ...
    @staticmethod
    def encoder(x, config, training, C, reuse=False, actv=tf.compat.v1.nn.relu, scope='image'):
        """
    ...
    seed=None
        ...
        def conv_block(x, filters, kernel_size=[3,3], strides=2, padding='same', actv=actv, init=init):
            ...
            x = tf.compat.v1.layers.conv2d(x, filters, kernel_size, strides=strides, padding=padding, activation=None)
            x = tf.compat.v1.layers.batch_normalization(x, **bn_kwargs)
            #x = tf.keras.layers.BatchNormalization(axis=1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', moving_mean_initializer='zeros', moving_variance_initializer='ones', beta_regularizer=None, gamma_regularizer=None, beta_constraint=None, gamma_constraint=None)
            #x = tf.contrib.layers.instance_norm(x, **in_kwargs)
            x = actv(x)
            return x

        with tf.compat.v1.variable_scope('encoder_{}'.format(scope), reuse=reuse):
    ...
    def quantizer(w, config, reuse=False, temperature=1, L=5, scope='image'):
        ...
        """
        with tf.compat.v1.variable_scope('quantizer_{}'.format(scope, reuse=reuse)):
            ...
            smx = tf.nn.softmax(-1.0/temperature * tf.abs(w_stack - centers))
    ...
    seed=None
            ...
            res = tf.compat.v1.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], 'REFLECT')
            res = tf.compat.v1.layers.conv2d(res, filters=n_filters, kernel_size=kernel_size, strides=strides,
                    activation=None, padding='VALID')
            res = actv(tf.compat.v1.layers.batch_normalization(res))

            res = tf.compat.v1.pad(res, [[0, 0], [p, p], [p, p], [0, 0]], 'REFLECT')
            res = tf.compat.v1.layers.conv2d(res, filters=n_filters, kernel_size=kernel_size, strides=strides,
                    activation=None, padding='VALID')
        ...
        def upsample_block(x, filters, kernel_size=[3,3], strides=2, padding='same', actv=actv, batch_norm=True):
            bn_kwargs = {'center':True, 'scale':True, 'training':training, 'fused':True, 'renorm':False}
            in_kwargs = {'center':True, 'scale': True}
            x = tf.compat.v1.layers.conv2d_transpose(x, filters, kernel_size, strides=strides, padding=padding, activation=None)
        ...
        with tf.compat.v1.variable_scope('decoder', reuse=reuse):
            w_bar = tf.compat.v1.pad(w_bar, [[0, 0], [1, 1], [1, 1], [0, 0]], 'REFLECT')
            ...
            ups = tf.compat.v1.pad(ups, [[0, 0], [3, 3], [3, 3], [0, 0]], 'REFLECT')
            ups = tf.compat.v1.layers.conv2d(ups, 3, kernel_size=7, strides=1, padding='VALID')

            out = tf.compat.v1.nn.tanh(ups)
    ...
    def multiscale_discriminator(x, config, training, actv=tf.nn.leaky_relu, use_sigmoid=False, 
        ...
        # Downsample input
        x2 = tf.compat.v1.layers.average_pooling2d(x, pool_size=3, strides=2, padding='same')
        x4 = tf.compat.v1.layers.average_pooling2d(x2, pool_size=3, strides=2, padding='same')
        ...
        def discriminator(x, scope, actv=actv, use_sigmoid=use_sigmoid, ksize=ksize, reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('discriminator_{}'.format(scope), reuse=reuse):
                c1 = tf.compat.v1.layers.conv2d(x, 64, kernel_size=ksize, strides=2, padding='same', activation=actv)
                ...
                out = tf.compat.v1.layers.conv2d(c4, 1, kernel_size=ksize, strides=1, padding='same')
    ...
    seed=None
        ...
        kwargs = {'center':True, 'scale':True, 'training':training, 'fused':True, 'renorm':False}
        with tf.compat.v1.variable_scope('noise_generator', reuse=reuse):

            # [batch_size, 4, 8, dim]
            with tf.compat.v1.variable_scope('fc1', reuse=reuse):
                h2 = tf.compat.v1.layers.dense(z, units=4 * 8 * upsample_dim, activation=actv, kernel_initializer=init)  # cifar-10
                h2 = tf.compat.v1.layers.batch_normalization(h2, **kwargs)
                h2 = tf.reshape(h2, shape=[-1, 4, 8, upsample_dim])

            # [batch_size, 8, 16, dim/2]
            with tf.compat.v1.variable_scope('upsample1', reuse=reuse):
                up1 = tf.compat.v1.layers.conv2d_transpose(h2, upsample_dim//2, kernel_size=kernel_size, strides=2, padding='same', activation=actv)
                up1 = tf.compat.v1.layers.batch_normalization(up1, **kwargs)

            # [batch_size, 16, 32, dim/4]
            with tf.compat.v1.variable_scope('upsample2', reuse=reuse):
                up2 = tf.compat.v1.layers.conv2d_transpose(up1, upsample_dim//4, kernel_size=kernel_size, strides=2, padding='same', activation=actv)
                up2 = tf.compat.v1.layers.batch_normalization(up2, **kwargs)
            
            # [batch_size, 32, 64, dim/8]
            with tf.compat.v1.variable_scope('upsample3', reuse=reuse):
                up3 = tf.compat.v1.layers.conv2d_transpose(up2, upsample_dim//8, kernel_size=kernel_size, strides=2, padding='same', activation=actv)  # cifar-10
                up3 = tf.compat.v1.layers.batch_normalization(up3, **kwargs)

            with tf.compat.v1.variable_scope('conv_out', reuse=reuse):
                out = tf.compat.v1.pad(up3, [[0, 0], [3, 3], [3, 3], [0, 0]], 'REFLECT')
                out = tf.compat.v1.layers.conv2d(out, C, kernel_size=7, strides=1, padding='VALID')
